...
const { editAdvertisements, resetAdvParams, setAdvParams, fetchMyAdvBySlug } = adv_store
...
const validate = () => {
  ...
  errors.file = !files.value && !advCreate.value.savedImages.length;
...
const createAdv = async () => {
  ...
  try {
    ...
    const data = await editAdvertisements(route.params.id, formData);
    loading.value = false
    if (data) {
      router.push(`/advertisement/my/${data?.data?.slug}`)
    }
  } catch (error) {
    console.log(error, 'create edit');
    alert(`ОШИБКА`)
    loading.value = false
  }
}
...
onMounted(async () => {
    resetAdvParams()
    try {
        await fetchMyAdvBySlug(route.params.id)
    } catch (error) {
        console.log('error');
        throw error
    }
    ...
    >
      ...
      <v-toolbar elevation="5" color="white">
        <v-container class="d-flex align-center justify-space-between">
          <div class="pa-3 ml-4 mr-5" @click="$router.push(localePath(`/advertisement/my/${route.params.id}`))">
            <v-img src="/svgIcon/arrow-left.svg" cover max-width="7" width="7" height="12"/>
          </div>
          <h3 class="text-body-1">{{$t('edit-advertisement')}}</h3>
      ...
      <v-container>
        ...
        <v-form ref="FormData" @submit.prevent="createAdv">
          ...
          <UIDropImage :oldImages="advCreate?.savedImages" :class="errors.file ? 'mb-2' : 'mb-8'" :uploadMsg="$t('add-photo')" :max="5 - advCreate?.savedImages?.length" :error="$t('photo-error')" @changed="handleImages" />
          ...
          <v-btn :loading="loading" size="x-large" color="primary" rounded="lg" elevation="0" class="text-none w-100" type="submit">Опубликовать</v-btn>
        </v-form>
