...
const deleteAdv = async () => {
  try {
    dialog.value = false;
    await deleteAdvertisements(route.params.id);
    router.push("/cabinet");
  } catch (error) {
    console.log(error);
  }
};
const archiveAdv = async () => {
  try {
    await archiveAdvertisements(route.params.id);
    router.push("/cabinet");
  } catch (error) {
    console.log(error);
  }
};
const unArchiveAdv = async () => {
  try {
    await unArchiveAdvertisements(route.params.id);
    router.push("/cabinet");
  } catch (error) {
    console.log(error);
  }
    ...
    >
      <v-toolbar elevation="5" color="white">
        <v-container class="d-flex align-center justify-space-between">
          <div
            class="pa-3 ml-4 mr-5"
            @click="$router.push(localePath('/cabinet'))"
          >
            <v-img
      ...
      <v-container class="pb-10" v-else>
        <div
        ...
        >
          ...
          <div class="d-flex align-center justify-space-between">
            <v-btn
              @click="
                $router.push(
                  localePath(`/advertisement/my/${route.params.id}/edit`)
                )
              "
    ...
    <v-dialog v-model="dialog" max-width="400" persistent>
      ...
      >
        <template v-slot:actions>
          ...
          <v-btn color="primary" variant="outlined" @click="deleteAdv()">
            Да
          </v-btn>
        </template>
      </v-card>
